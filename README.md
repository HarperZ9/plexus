...
The built-in manifests are committed under [`manifests/`](manifests/) and were
generated by `plexus export`: the exact files each tool ships. Discovery from
those JSON files produces the same mesh as the built-in registry (a round-trip
test enforces it), so the format faithfully represents a real tool.
